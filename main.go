...
func SetDingToken(token string) {
	_log.DingToken(token)
}
...
func (l *logger) DingToken(token string) {
	l.dingtoken = token
}

func (l *logger) send(msg string) error {
	if l.dingtoken != "" {
		resp, err := http.Post("https://oapi.dingtalk.com/robot/send?access_token=" + l.dingtoken,
